...
impl WebProxy {
    ...
    pub async fn start(&self, bind_address: &str, port: u16) -> Result<()> {
        ...
        let listener = TcpListener::bind(addr).context("Failed to bind proxy server")?;
                ...
                FilterAction::Allow => {
                    ...
                    match Self::establish_tunnel(req, host, port, ca_cert_path, ca_key_path).await {
                        ...
                        Err(e) => {
                            ...
                            Self::create_error_response(&format!("Failed to connect to {}:{}", host, port))
                        }
                    }
                }
                FilterAction::Block => {
                ...
                FilterAction::Warn => {
                    ...
                    match Self::establish_tunnel(req, host, port, ca_cert_path, ca_key_path).await {
                        Ok(()) => Ok(Response::builder()
                            ...
                            .unwrap()),
                        Err(e) => {
                            Self::create_error_response(&format!("Failed to connect to {}:{}", host, e))
                        }
                    }
                }
            },
    ...
    async fn establish_tunnel(
        ...
        let io_target = TokioIo::new(target_stream);
        ...
        // Extract the client stream from the request
        // For hyper 1.0, we need to handle this differently
        // The connection is already being served, so we need to work with what we have

        // For now, just establish a simple tunnel without SSL interception
        // SSL interception would require more complex certificate generation
    ...
    /// Create a redirect response for safe search
...
impl WebProxy {
    /// Create a new web proxy with the given filter engine
    pub fn new(filter_engine: FilterEngine) -> Self {
        Self { filter_engine: Arc::new(filter_engine) }
    }
    ...
    pub async fn start(&self, bind_address: &str, port: u16) -> Result<()> {
        ...
        loop {
            ...
            tokio::task::spawn(async move {
                let service =
                    service_fn(move |req| Self::handle_request(Arc::clone(&filter_engine), req));
    ...
    async fn handle_request(
        ...
        match req.method() {
            &Method::CONNECT => Self::handle_connect_request(filter_engine, req).await,
            _ => Self::handle_http_request(filter_engine, req).await,
        }
    }
        ...
        match filter_engine.evaluate_request(&test_url, "CONNECT").await {
            Ok(decision) => match decision.action {
                FilterAction::Allow => {
                    ...
                    // In a full implementation, we would establish a tunnel
                    Ok(Response::builder()
                        .status(StatusCode::OK)
                        .body(Empty::new().map_err(|never| match never {}).boxed())
                        .unwrap())
    ...
    async fn handle_http_request(
        ...
        // Evaluate the URL against our filters
        match filter_engine.evaluate_request(&url, method).await {
            Ok(decision) => match decision.action {
                FilterAction::Allow => {
                    debug!("Allowing request to {}", url);

                    // Check for safe search rewriting
                    if let Some(safe_url) = filter_engine.rewrite_url_for_safe_search(&url).await {
                        if safe_url != url {
                            debug!("Rewriting URL for safe search: {} -> {}", url, safe_url);
                            return Self::create_redirect_response(&safe_url);
                        }
                    }
                ...
                FilterAction::Block => {
                    ...
                    Self::create_block_response(
                        &url,
                        &decision.reason,
                        decision.category.as_deref().unwrap_or("blocked"),
                    )
                }
                FilterAction::Warn => {
                    warn!("Warning for request to {} - {}", url, decision.reason);
                    // For now, treat warn as allow but could show warning page
                    Self::forward_request(req).await
                }
            },
            Err(err) => {
